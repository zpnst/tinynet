...
print_device(FILE *dotf, abs_dev_t *dev, int show_ip, int show_mac)
{
    ...
    const tinynet_char_t *shape = get_shape_by_device_type(dev->basic_info.dev_type);
    const tinynet_char_t *color = get_color_by_device_type(dev->basic_info.dev_type);

    tinynet_char_t ip_str[IP_BUFFER_S];
    tinynet_char_t mac_str[MAC_BUFFER_S];

    tinynet_char_t label[256];
